# 🩺 Health Butler（健康管家）

[![Vercel](https://img.shields.io/badge/Deployed%20on-Vercel-black?logo=vercel)](https://hearth-bulter.vercel.app)
[![Next.js](https://img.shields.io/badge/Next.js-15-black?logo=next.js)](https://nextjs.org/)
[![License](https://img.shields.io/badge/License-MIT-blue.svg)](LICENSE)

> 让个人与家庭的健康管理从「主观感性」走向「客观数据驱动」

🚀 **生产环境**: [https://hearth-bulter.vercel.app](https://hearth-bulter.vercel.app)

**Health Butler** 是一个基于健康数据与电商库存的动态饮食引擎，通过AI营养规划、智能食谱生成和电商自动采购，帮助家庭建立可持续的健康管理习惯。

---

## ✨ 核心特性

### 🧬 数据驱动的营养规划
- 接入体检报告（OCR识别）和可穿戴设备数据
- 基于BMR/TDEE自动计算个性化宏量营养素分配
- 支持不同健康目标（减重/增肌/维持/疾病管理）

### 🍱 智能食谱生成
- 7天/30天/90天周期性食谱计划
- 动态适配季节、地区与电商平台库存
- 考虑家庭成员差异（老年人降钠、儿童补钙等）

### 🛒 电商自动采购
- 一键生成购物清单并匹配SKU
- 对接山姆/盒马/叮咚买菜（Roadmap）
- 预算控制 + 食材保质期提醒

### 📊 健康数据闭环
- 可视化体重/体脂/营养摄入趋势
- AI分析营养失衡并自动调整下轮食谱
- 生成季度健康报告

---

## 🏗️ 技术架构

```
┌─────────────────────────────────────────────────┐
│  Frontend (Next.js 15 + React 18 + TypeScript)  │
├─────────────────────────────────────────────────┤
│  API Layer (Next.js API Routes / tRPC)          │
├─────────────────────────────────────────────────┤
│  Business Logic (Services)                      │
│  ├─ 营养计算引擎 (Macro Calculator)              │
│  ├─ 食谱生成器 (Meal Planner)                    │
│  └─ SKU匹配服务 (Inventory Mapper)               │
├─────────────────────────────────────────────────┤
│  Data Access Layer (Prisma ORM)                 │
├─────────────────────────────────────────────────┤
│  Database (PostgreSQL 16)                       │
└─────────────────────────────────────────────────┘

External APIs:
- USDA FoodData Central (营养数据库)
- OCR Service (体检报告识别)
- Wearable Device SDKs (Apple Health / 华为Health)
```

### 技术栈

**前端**:
- Next.js 15 (App Router), TypeScript 5.x
- React 18, Tailwind CSS 4.x, shadcn/ui
- Zustand (状态管理), React Hook Form + Zod

**后端**:
- Node.js 20+ LTS, Prisma 6.x
- PostgreSQL 16, NextAuth.js v5

**AI/ML**:
- OpenAI GPT-4 / Anthropic Claude (营养建议)
- Tesseract.js / Azure OCR (文字识别)

**部署**:
- ✅ Vercel (前端 + Serverless Functions + Edge Functions)
- ✅ Neon PostgreSQL (Serverless 数据库)
- ✅ GitHub + Vercel 自动部署 (CI/CD)

---

## 🚀 快速开始

### 前置要求

- Node.js 20+ LTS
- PostgreSQL 16
- pnpm (推荐) 或 npm

### 安装步骤

```bash
# 1. 克隆仓库
git clone https://github.com/yourusername/HearthBulter.git
cd HearthBulter

# 2. 安装依赖
pnpm install

# 3. 配置环境变量
cp .env.example .env
# 编辑 .env 文件，填入数据库连接和API密钥

# 必需的环境变量：
# DATABASE_URL - PostgreSQL数据库连接字符串
# NEXTAUTH_SECRET - NextAuth.js密钥（至少32个字符）
# NEXTAUTH_URL - 应用URL（开发环境: http://localhost:3000）

# 可选的环境变量：
# USDA_API_KEY - USDA FoodData Central API密钥（用于营养数据查询）
# REDIS_URL - Redis连接URL（用于缓存，可选）
# GOOGLE_CLIENT_ID - Google OAuth客户端ID（可选）
# GOOGLE_CLIENT_SECRET - Google OAuth客户端密钥（可选）

## 获取USDA API Key

1. 访问 [USDA FoodData Central API](https://fdc.nal.usda.gov/api-guide.html)
2. 注册账号并登录
3. 在API Key页面申请免费API密钥
4. 将密钥添加到 `.env` 文件：
   ```
   USDA_API_KEY=your-usda-api-key-here
   ```

**注意**: USDA API免费额度为每小时1000次请求。如果未配置API Key，系统将仅使用本地数据库中的食物数据。

# 4. 初始化数据库
pnpm prisma migrate dev

# 5. 启动开发服务器
pnpm dev
```

访问 `http://localhost:3000` 查看应用。

---

## 📂 项目结构

```
HearthBulter/
├── src/
│   ├── app/                    # Next.js App Router 页面
│   │   ├── (auth)/            # 认证页面
│   │   ├── dashboard/         # 主仪表盘
│   │   └── api/               # API路由
│   ├── components/            # React组件
│   │   ├── ui/               # shadcn/ui基础组件
│   │   └── features/         # 业务组件
│   ├── lib/                   # 核心库
│   │   ├── db/               # Prisma客户端
│   │   ├── services/         # 业务逻辑服务
│   │   └── utils/            # 工具函数
│   ├── types/                # TypeScript类型定义
│   └── hooks/                # 自定义Hooks
├── prisma/
│   └── schema.prisma         # 数据库Schema
├── openspec/                  # OpenSpec规范文档
│   ├── project.md            # 项目上下文
│   ├── specs/                # 功能规范
│   └── changes/              # 变更提案
├── tests/                     # 测试文件
│   ├── unit/                 # 单元测试
│   └── e2e/                  # E2E测试
└── docs/                      # 项目文档
```

---

## 🧩 核心模块

| 模块 | 状态 | 说明 |
|------|------|------|
| 家庭档案管理 | 🟡 Planning | 成员信息、健康目标、过敏史 |
| 营养数据库 | 🟡 Planning | USDA食物数据对接 |
| 健康数据采集 | 🟡 Planning | 手动录入 + 可穿戴设备同步 |
| 体检报告接入 | 🟡 Planning | OCR识别 + 结构化存储 |
| 食谱规划引擎 | 🟡 Planning | 模板化7天食谱生成 |
| 购物清单生成 | 🟡 Planning | 自动提取食材 + SKU匹配 |
| 健康追踪仪表盘 | 🟡 Planning | 趋势可视化 + 营养分析 |

图例: 🟢 已完成 | 🟡 规划中 | 🔴 未开始

---

## 📋 开发规范

本项目采用 **OpenSpec** 规范驱动开发流程：

1. **创建变更提案** (`openspec/changes/[name]/`)
   - 编写 `proposal.md` 说明变更原因和影响
   - 在 `specs/` 中定义增量需求（ADDED/MODIFIED/REMOVED）
   - 创建 `tasks.md` 实现清单

2. **验证规范**
   ```bash
   openspec validate --strict
   ```

3. **实现功能**
   - 按照 `tasks.md` 顺序完成
   - 编写单元测试（覆盖率 >80%）
   - 提交PR并通过CI/CD检查

4. **归档变更**
   ```bash
   openspec archive [change-name]
   ```

详见 [OpenSpec文档](./openspec/AGENTS.md)

### 🔍 代码审查

项目集成了自动化代码审查系统，确保代码质量和一致性：

**审查维度**:
- **复杂度分析**: 检测函数复杂度，防止代码难以维护
- **类型安全**: 检查TypeScript类型使用，减少运行时错误
- **安全漏洞**: 扫描SQL注入、XSS等常见安全问题
- **代码风格**: 统一命名规范和编码风格
- **性能优化**: 识别潜在的性能问题

**使用方法**:

```bash
# 运行代码审查（开发环境）
pnpm lint

# 生成审查报告
node scripts/generate-review-report.js

# 查看审查配置
cat src/lib/code-review-config.ts
```

**审查规则**:
- ✅ 通过审查: 提交正常进行
- ⚠️ 轻微问题: 允许提交但建议修复
- 🚫 严重问题: 阻止提交，必须修复

**CI/CD集成**:
- Pre-commit钩子自动审查
- GitHub Actions PR审查
- 审查报告自动生成

---

## 🧪 测试

```bash
# 单元测试
pnpm test

# 单元测试（watch模式）
pnpm test:watch

# E2E测试
pnpm test:e2e

# 测试覆盖率
pnpm test:coverage
```

**测试要求**:
- 核心业务逻辑必须有单元测试（>80%覆盖率）
- 关键用户流程必须有E2E测试
- CI/CD中测试失败将阻止合并

---

## 🤝 贡献指南

我们欢迎所有形式的贡献！请遵循以下步骤：

1. Fork本仓库
2. 创建功能分支 (`git checkout -b feature/amazing-feature`)
3. 提交变更 (`git commit -m 'feat: add amazing feature'`)
4. 推送到分支 (`git push origin feature/amazing-feature`)
5. 创建Pull Request

**提交规范**: 遵循 [Conventional Commits](https://www.conventionalcommits.org/)

---

## 🛣️ Roadmap

### MVP阶段 (当前)
- [x] 项目初始化 + OpenSpec架构建立
- [ ] 家庭档案管理
- [ ] 健康数据录入
- [ ] 营养数据库对接
- [ ] 模板化食谱生成
- [ ] 购物清单生成

### 阶段2 (Q2 2025)
- [ ] 体检报告OCR识别
- [ ] 可穿戴设备数据同步
- [ ] 电商API对接（叮咚买菜）
- [ ] AI食谱优化

### 阶段3 (Q3 2025)
- [ ] 多平台SKU智能匹配
- [ ] 高级营养分析报告
- [ ] 家庭健康趋势预测
- [ ] 移动端APP (React Native)

### 阶段4 (Q4 2025)
- [ ] 医疗机构合作（体检中心API）
- [ 商业化（订阅制 + 电商返佣）
- [ ] 智能家居设备集成

---

## 📄 许可证

本项目采用 [MIT License](LICENSE) 开源协议。

---

## 👨‍💻 作者

**Ronn Huang** - [GitHub](https://github.com/yourusername)

---

## 🙏 致谢

- [USDA FoodData Central](https://fdc.nal.usda.gov/) - 提供营养数据API
- [shadcn/ui](https://ui.shadcn.com/) - 优秀的UI组件库
- [OpenSpec](https://github.com/openspec-dev/openspec) - 规范驱动开发框架

---

**⭐ 如果这个项目对你有帮助，请给我们一个Star！**
