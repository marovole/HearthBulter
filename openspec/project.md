...
### 后端（当前架构：Convex + Clerk + Cloudflare Pages）

- **运行时**: Node.js 20+ LTS / Cloudflare Workers
- **框架**: Next.js API Routes
- **数据库**: Convex（实时数据库，Schema 在 `convex/schema.ts`）
- **认证**: Clerk 6.x（NextAuth 已于 2026-01-27 迁移）
- **文件存储**: Convex file storage

> **历史注:** Prisma/PostgreSQL/Neon 已于 2026-01-18 迁移至 Supabase，再于 2026-01-27 迁移至 Convex。NextAuth 已迁移至 Clerk。

### AI/ML

- **LLM**: OpenAI GPT-4 / OpenRouter (营养规划)
- **OCR**: Tesseract.js (体检单识别)
- **营养数据**: USDA FoodData Central API

### 部署与监控

- **托管**: Cloudflare Pages（全球 CDN + Edge Functions）
- **CI/CD**: GitHub Actions
- **监控**: Sentry
- **实时**: Convex subscriptions
...
├─────────────────────────────────┤
│  Data Access Layer              │  Convex mutations/queries
├─────────────────────────────────┤
...
- **Convex Functions**: 数据访问通过 `convex/` 目录的 mutation/query
- **Service Pattern**: 业务逻辑封装（营养计算、食谱生成）
- **Factory Pattern**: 食谱生成器（不同目标不同策略）
- **Observer Pattern**: Convex 实时订阅触发 UI 更新

**数据模型规范**:

- 使用 `convex/schema.ts` 定义所有数据模型
- 时间戳字段: `_creationTime`（Convex 自动管理）
