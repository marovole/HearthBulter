    ...
    for (const line of lines) {
      ...
      if (commitMatch) {
        ...
        currentCommit = {
          ...
          message,
          date: '', // 将在后续获取
          batchNumber: batchMatch ? parseInt(batchMatch[1]) : undefined,
          files: [],
        };
      ...
      if (currentCommit && filePath.startsWith('src/app/api/') && filePath.endsWith('.ts')) {
...
function getCommitDate(hash: string): string {
  try {
    const date = execSync(`git show -s --format=%ci ${hash}`, { encoding: 'utf-8' }).trim();
 ...
 */
function extractRouteInfo(filePath: string, content: string): { methods: string[]; route: string } {
 ...
 * 检测文件中的技术栈使用
 */
function detectTechStack(filePath: string, content: string): {
  supabase: boolean;
  prismaClient: boolean;
  prismaAdapter: boolean;
  ...
  for (const line of lines) {
    ...
    // 检测导入
    if (trimmed.includes("from '@prisma/client'") || trimmed.includes("from \"@prisma/client\"")) {
      detections.prismaClient = true;
    }
    if (trimmed.includes('supabase-adapter')) {
      detections.prismaAdapter = true;
    }
    if (trimmed.includes('/db/supabase-adapter') || trimmed.includes('/db/supabase')) {
 ...
 * 从文件中提取代码样本
 */
function extractCodeSamples(content: string): { supabase?: string; prisma?: string } {
  const lines = content.split('\n');
  const samples: { supabase?: string; prisma?: string } = {};

  // 提取supabase查询样本
  const supabaseLineIndex = lines.findIndex((line) => line.includes('supabase.from('));
...
function generateReport(commits: MigrationCommit[]): AnalysisReport {
  ...
  for (const ep of endpoints) {
    report.migrationSummary[ep.migrationStatus.toLowerCase() as keyof typeof report.migrationSummary]++;
    report.riskSummary[ep.riskLevel.toLowerCase() as keyof typeof report.riskSummary]++;
  }
...
function printReport(report: AnalysisReport): void {
  ...
  // 迁移状态摘要
  console.log('\n📊 迁移状态分布:');
  console.log(`  ${colors.green('✓ FULL (完全迁移):')} ${report.migrationSummary.full}`);
  console.log(`  ${colors.yellow('⚠ PARTIAL (部分迁移):')} ${report.migrationSummary.partial}`);
  console.log(`  ${colors.red('✗ NONE (未迁移):')} ${report.migrationSummary.none}`);
  console.log(`  ${colors.red('✗ FAKE (假迁移 ⭐ 严重):')} ${report.migrationSummary.fake}`);
  ...
  // 高风险端点详情
  const highRiskEndpoints = report.endpoints.filter((ep) => ep.riskLevel === 'HIGH');
  if (highRiskEndpoints.length > 0) {
    ...
    for (const ep of highRiskEndpoints) {
      ...
      console.log(`  使用Supabase: ${ep.usesSupabase ? colors.green('是') : colors.red('否')}`);
      console.log(`  使用Prisma: ${ep.usesPrismaDirectly ? colors.red('直接导入 ⭐ 严重') : ep.usesPrismaFromAdapter ? colors.yellow('通过Adapter') : colors.gray('否')}`);
      ...
      if (ep.codeSamples.prisma) {
        ...
        console.log(colors.gray(ep.codeSamples.prisma.split('\n').map((l) => `    ${l}`).join('\n')));
      }
    }
  }
  ...
  for (const ep of report.endpoints) {
    const statusIcon = ep.migrationStatus === 'FULL' ? '✓' : ep.migrationStatus === 'FAKE' ? '✗' : '~';
    const statusColor = ep.migrationStatus === 'FULL' ? colors.green : ep.migrationStatus === 'FAKE' ? colors.red : colors.yellow;
    const riskColor = ep.riskLevel === 'LOW' ? colors.green : ep.riskLevel === 'MEDIUM' ? colors.yellow : colors.red;

    console.log(`${statusColor(`${statusIcon} [${ep.migrationStatus}]`)} ${ep.route} ${riskColor(`(${ep.riskLevel})`)}`);
    console.log(colors.gray(`   └─ ${ep.filePath}`));
    console.log(colors.gray(`   └─ Supabase: ${ep.calls.supabaseQueries} 次, Prisma: ${ep.calls.prismaQueries} 次`));
...
function main(): void {
  ...
  for (const commit of commits) {
    if (commit.batchNumber) {
      console.log(colors.gray(`  └─ Batch ${commit.batchNumber}: ${commit.hash} - ${commit.message}`));
    } else {
