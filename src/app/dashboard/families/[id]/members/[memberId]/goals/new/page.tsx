...
export default function NewGoalPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/members/${memberId}/goals`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          targetWeeks: formData.targetWeeks ? parseInt(formData.targetWeeks) : undefined,
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 justify-between">
            <div className="flex items-center">
              ...
              <h1 className="text-xl font-semibold text-gray-900">新建健康目标</h1>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="px-4 py-6 sm:px-0">
          <div className="rounded-lg bg-white p-6 shadow">
            <form onSubmit={handleSubmit} className="space-y-6">
              {/* 目标类型 */}
              <div>
                <label htmlFor="goalType" className="mb-2 block text-sm font-medium text-gray-700">
                  目标类型 *
                </label>
                <select
                  id="goalType"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, targetWeeks: e.target.value })}
                />
                <p className="mt-1 text-sm text-gray-500">预计达成目标需要的周数（1-52周）</p>
              </div>
              ...
              <div className="border-t border-gray-200 pt-6">
                <h3 className="mb-4 text-lg font-medium text-gray-900">营养比例配置</h3>
                ...
                <div className="mb-4">
                  <div className="mb-2 flex items-center justify-between">
                    <label className="block text-sm font-medium text-gray-700">碳水化合物</label>
                ...
                <div className="mb-4">
                  <div className="mb-2 flex items-center justify-between">
                    <label className="block text-sm font-medium text-gray-700">蛋白质</label>
                    <span className="text-sm font-medium text-gray-900">
                ...
                <div className="mb-4">
                  <div className="mb-2 flex items-center justify-between">
                    <label className="block text-sm font-medium text-gray-700">脂肪</label>
                    <span className="text-sm font-medium text-gray-900">
                ...
                <div
                  className={`rounded-md p-3 ${
                    Math.abs(formData.carbRatio + formData.proteinRatio + formData.fatRatio - 1) <
                    0.01
                ...
                >
                  <p className="text-sm font-medium">
                    总计:{" "}
                    {(
                      (formData.carbRatio + formData.proteinRatio + formData.fatRatio) *
                      100
                    ).toFixed(0)}
                    %
                    {Math.abs(formData.carbRatio + formData.proteinRatio + formData.fatRatio - 1) >=
              ...
              <div className="flex gap-4 pt-4">
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="flex-1 rounded-md border border-transparent bg-blue-600 px-4 py-2 text-white shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  {isLoading ? "创建中..." : "创建目标"}
                </button>
              </div>
