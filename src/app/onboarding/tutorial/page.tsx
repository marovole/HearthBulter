...
const tutorialSteps: TutorialStep[] = [
  ...
  },
  {
    ...
    content: {
      ...
      tips: ["清单按商店区域分类，方便购物", "可以设置提醒避免遗漏", "支持多人协作编辑清单"],
    },
  },
  {
    id: "device-sync",
    ...
    content: {
      ...
      tips: ["确保设备蓝牙已开启", "首次连接可能需要较长时间", "可以手动触发同步或设置自动同步"],
    },
  },
];
...
export default function TutorialPage() {
  ...
  const currentTutorial: TutorialStep = tutorialSteps[currentStep] ?? tutorialSteps[0]!;

  const handleNext = () => {
    if (currentStep < tutorialSteps.length - 1) {
      if (currentTutorial.id) {
        markStepComplete(currentTutorial.id);
      }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 px-4 py-8">
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="mb-2 text-3xl font-bold text-gray-900">功能教程</h1>
            <p className="text-gray-600">学习如何使用 Health Butler 的核心功能</p>
          </div>
          ...
          {tutorialSteps.map((tutorial, index) => (
            ...
            >
              <div className="flex items-center space-x-2">
                {completedSteps.has(tutorial.id) && <CheckCircle className="h-4 w-4" />}
                <span>
        ...
        <Card className="mb-8">
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="mb-2 text-2xl">{currentTutorial.title}</CardTitle>
          ...
          <CardContent className="space-y-6">
            {/* Overview */}
            <div>
              <h3 className="mb-3 text-lg font-semibold">功能概述</h3>
              <p className="leading-relaxed text-gray-700">{currentTutorial.content.overview}</p>
            </div>

            {/* Steps */}
            <div>
              <h3 className="mb-3 text-lg font-semibold">操作步骤</h3>
            ...
            {/* Tips */}
            <div>
              <h3 className="mb-3 text-lg font-semibold">使用技巧</h3>
            ...
            <div className="rounded-lg bg-gray-50 p-6 text-center">
              <h4 className="mb-2 font-medium">实践练习</h4>
              <p className="mb-4 text-gray-600">
                现在您已经了解了基本操作，让我们实际体验一下这个功能。
              </p>
              <Button className="bg-blue-600 hover:bg-blue-700">开始实践</Button>
            </div>
          </CardContent>
        </Card>
