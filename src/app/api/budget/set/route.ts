...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!periodValidation.success) {
      return NextResponse.json({ error: "无效的预算周期" }, { status: 400 });
    }
    ...
    if (total <= 0) {
      return NextResponse.json({ error: "总预算必须大于0" }, { status: 400 });
    }
    ...
    return NextResponse.json({ error: "创建预算失败" }, { status: 500 });
  }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: "获取预算列表失败" }, { status: 500 });
  }
}
