...
const validCategories = new Set(Object.values(FoodCategory));
...
export async function GET(
  ...
  try {
    ...
    if (!validCategories.has(category as FoodCategory)) {
      return NextResponse.json({ error: "无效的食物分类" }, { status: 400 });
    }

    const from = (page - 1) * limit;
    const to = from + limit - 1;
    const categoryValue = category as FoodCategory;

    const [foods, total] = await Promise.all([
      foodRepository.listByCategory(categoryValue, from, to),
      foodRepository.countByCategory(categoryValue),
    ]);
