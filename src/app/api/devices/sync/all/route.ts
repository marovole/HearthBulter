...
export async function POST(request: NextRequest) {
  try {
    ...
    if (validatedData.memberId) {
      ...
      memberIds = accessibleMembers.map((member) => member._id as Id<"familyMembers">);
    }
    ...
    devicesToSync = devicesToSync.filter(
      (device) => device.isAutoSync !== false && device.syncStatus !== "DISABLED"
    );

    if (validatedData.platforms && validatedData.platforms.length > 0) {
      const platforms = new Set(validatedData.platforms);
      devicesToSync = devicesToSync.filter((device) => platforms.has(String(device.platform)));
    }
    ...
    const deviceUpdatePromises = devicesToSync.map(async (deviceRecord: DeviceConnectionRecord) => {
      ...
      try {
        ...
        syncResult = {
          ...
          syncedCount: 0,
          skippedCount: 0,
          errors: [updateError instanceof Error ? updateError.message : "同步失败"],
          lastSyncDate: new Date(),
        };
      }
  ...
  try {
    ...
    const history = await Promise.all(
      (syncHistory || []).map(async (record) => {
        ...
        if (!member) {
          const memberDoc = await convexClient.query<Record<string, unknown> | null>(
            api.members.getById,
            {
        ...
        if (deviceIdValue) {
          ...
          if (!deviceConnection) {
            const deviceDoc = await convexClient.query<Record<string, unknown> | null>(
              api.devices.getById,
              {
