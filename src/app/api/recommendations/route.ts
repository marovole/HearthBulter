...
export async function GET(request: NextRequest) {
  try {
    ...
    const mealType =
      mealTypeParam && ["BREAKFAST", "LUNCH", "DINNER", "SNACK"].includes(mealTypeParam)
    ...
    const season =
      seasonParam && ["SPRING", "SUMMER", "AUTUMN", "WINTER"].includes(seasonParam)
    ...
    const recommendationEngine = new RecommendationEngine(recommendationRepository);
    const recommendations: RecipeRecommendation[] = await recommendationEngine.getRecommendations(
      context,
      limit
    );
