...
export async function PUT(
  ...
  return withApiPermissions(async (req, context) => {
    try {
      ...
      const updatedItem = await ShoppingListService.updateShoppingItem(familyId, userId, itemId, {
        amount,
        estimatedPrice,
        assigneeId
      })
  ...
  return withApiPermissions(async (req, context) => {
    try {
      ...
      return NextResponse.json(
        { 
          success: false, 
          error: error instanceof Error ? error.message : 'Failed to delete shopping item' 
        },
        { status: 500 }
      )
    }
