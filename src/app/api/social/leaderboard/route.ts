...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!typeParam || !Object.values(LeaderboardType).includes(typeParam as LeaderboardType)) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const leaderboard = await service.getLeaderboard(type, undefined, timeframe || "weekly", 100);
 ...
 */
async function saveLeaderboardData(_leaderboard: unknown, _timeframe: string): Promise<void> {
  return;
}
