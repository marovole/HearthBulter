  ...
  try {
    ...
    if (token) {
      ...
      const { data: shareContent } = await supabase
        ...
        .eq('id', verification.payload.resourceId)
        .single();
    ...
    if (analysisType === 'user' && memberId) {
      ...
      if (statsResult.success && statsResult.data) {
        ...
        analytics = await mapRpcToShareAnalytics(
          statsResult.data,
          memberId,
          supabase,
        );
        ...
        additionalStats = calculateAdditionalStatsFromRpc(
          statsResult.data,
          analytics,
        );
      } else {
        // RPC 失败，降级到原有服务层
        console.warn(
          'RPC calculate_social_stats failed, falling back to service layer:',
          statsResult.error,
        );
        ...
        additionalStats = await getUserAdditionalStats(
          supabase,
          memberId,
          period,
        );
...
async function getGlobalAdditionalStats(
  ...
  (allSharesForGrouping || []).forEach((share) => {
    shareCountByMember[share.memberId] =
      (shareCountByMember[share.memberId] || 0) + 1;
  });

  const activeSharers = Object.values(shareCountByMember).filter(
    (count) => count >= 3,
  ).length;
...
async function mapRpcToShareAnalytics(
  ...
  const mappedPlatformBreakdown: Record<
    string,
    {
      shares: number;
      clicks: number;
  ...
  // 为保持向后兼容，需要重新计算
  const consistentConversionRate =
