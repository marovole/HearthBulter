...
import { Permission, FamilyMemberRole } from "@/lib/permissions";
import { useFamilyMembers } from "@/hooks/use-family-members";
import { usePermissions } from "@/hooks/use-permissions";

type PermissionMatrix = Partial<Record<FamilyMemberRole, Permission[]>>;
  ...
  const exportPermissions = async () => {
    try {
      const config = {
        ...
        permissionMatrix,
        members: members.map((member: FamilyMemberInfo) => ({
          id: member.id,
          name: member.name,
          role: member.role,
        })),
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">所有端点受保护</p>
              </CardContent>
            </Card>
          </div>
