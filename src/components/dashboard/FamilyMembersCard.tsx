      ...
      <div className="space-y-2">
        ...
        {member.healthScore && (
          <div className="flex items-center justify-between">
            ...
            <span className={`px-2 py-1 rounded-full text-xs font-medium ${getHealthScoreColor(member.healthScore)}`}>
              {member.healthScore}
            </span>
          </div>
        )}
            ...
            {member.healthGoals.slice(0, 2).map((goal, index) => (
              <div key={index} className="flex items-center justify-between text-xs mb-1">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-gray-600">{Math.round((goal.current / goal.target) * 100)}%</span>
                </div>
              </div>
            ))}
          </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {members.map((member) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {member.healthScore && (
                      <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getHealthScoreColor(member.healthScore)}`}>
                        {member.healthScore}
                      </span>
                    )}
                  </td>
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xl font-semibold text-gray-900">
                {Math.round(members.reduce((acc, m) => acc + (m.healthScore || 0), 0) / members.length)}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow p-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xl font-semibold text-gray-900">
                {members.filter(m => m.lastActive && m.lastActive > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)).length}
              </p>
            </div>
          </div>
        </div>
