...
export function EcommerceIntegration({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="flex max-h-[80vh] w-full max-w-4xl flex-col rounded-lg bg-white">
        {/* Header */}
        <div className="flex items-center justify-between border-b p-6">
          <div>
            <h2 className="text-xl font-bold text-gray-900">购买 {foodName}</h2>
            <p className="mt-1 text-sm text-gray-500">
              需要数量: {amount >= 1000 ? `${(amount / 1000).toFixed(1)}kg` : `${amount}g`}
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loading ? (
            ...
            <div className="py-12 text-center">
              <div className="mb-4 text-gray-500">暂无匹配的商品</div>
              <p className="text-sm text-gray-400">可以尝试其他平台或调整搜索条件</p>
            </div>
          ) : (
                  ...
                  <div className="flex space-x-4">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="mb-2 flex items-start justify-between">
                        ...
                        <span
                          className={`rounded-full px-2 py-1 text-xs ${getAvailabilityColor(product.availability)}`}
                        >
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        >
                          {product.availability === "OUT_OF_STOCK" ? "缺货" : "选择"}
                        </button>
                      </div>
                    </div>
                  </div>
