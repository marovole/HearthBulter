...
describe('api_devices_route', () => {
  ...
  describe('GET', () => {
    it('should handle GET request', async () => {
      ...
      const { GET } = require('@/app/api/devices/route');
      ...
      const { req, res } = createMocks({
        method: 'GET',
        url: '/devices/route.ts',
      });

      // Execute
      // const response = await GET(new NextRequest('http://localhost:3000/devices/route.ts'), { params: {} });
  ...
  describe('POST', () => {
    it('should handle POST request', async () => {
      ...
      // const { POST } = require('@/app/api/devices/route');
