...
describe('api_devices_sync_route', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  describe('GET', () => {
    it('should handle GET request', async () => {
      ...
      const { GET } = require('@/app/api/devices/sync/route');
      ...
      const { req, res } = createMocks({
        method: 'GET',
        url: '/devices/sync/route.ts',
      });

      // Execute
      // const response = await GET(new NextRequest('http://localhost:3000/devices/sync/route.ts'), { params: {} });
  ...
  describe('POST', () => {
    it('should handle POST request', async () => {
      ...
      // const { POST } = require('@/app/api/devices/sync/route');
