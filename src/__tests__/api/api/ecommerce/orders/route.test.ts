...
describe('api_ecommerce_orders_route', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  describe('GET', () => {
    it('should handle GET request', async () => {
      ...
      const { GET } = require('@/app/api/ecommerce/orders/route');
      ...
      const { req, res } = createMocks({
        method: 'GET',
        url: '/ecommerce/orders/route.ts',
      });

      // Execute
      // const response = await GET(new NextRequest('http://localhost:3000/ecommerce/orders/route.ts'), { params: {} });
