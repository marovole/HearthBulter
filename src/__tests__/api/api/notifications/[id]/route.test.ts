/**
 * api/notifications/[id]/route.ts API 测试
...
describe('api_notifications_[id]_route', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  describe('GET', () => {
    it('should handle GET request', async () => {
      ...
      const { GET } = require('@/app/api/notifications/[id]/route');
      ...
      const { req, res } = createMocks({
        method: 'GET',
        url: '/notifications/$1/route.ts',
      });

      // Execute
      // const response = await GET(new NextRequest('http://localhost:3000/notifications/$1/route.ts'), { params: {} });
  ...
  describe('POST', () => {
    it('should handle POST request', async () => {
      ...
      // const { POST } = require('@/app/api/notifications/[id]/route');
