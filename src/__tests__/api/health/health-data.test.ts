...
jest.mock("@/lib/services/health-calculator", () => {
  const calculateBMI = jest.fn();
  const calculateBMR = jest.fn();
  const calculateTDEE = jest.fn();
  const calculateHealthScore = jest.fn();
  const analyzeHealthTrends = jest.fn();
...
describe("/api/health API", () => {
    ...
    it("should update health data entry", async () => {
      ...
      try {
        ...
        const response = await PUT(request, {
          params: Promise.resolve({ id: "health-1" }),
        });
  ...
  describe("Error handling", () => {
    ...
    it("should handle invalid date formats", async () => {
      const request = new NextRequest(
        "http://localhost:3000/api/health/data?startDate=invalid-date",
        {
          method: "GET",
          headers: {
            Authorization: "Bearer valid-jwt-token",
