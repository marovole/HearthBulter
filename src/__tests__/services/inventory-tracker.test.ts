...
describe("InventoryTracker", () => {
  ...
  describe("createInventoryItem", () => {
    ...
    it("should throw error for invalid food ID", async () => {
      ...
      await expect(inventoryTracker.createInventoryItem(itemData)).rejects.toThrow();
    });
  ...
  describe("useInventory", () => {
    ...
    it("should throw error for insufficient quantity", async () => {
      await expect(
        inventoryTracker.useInventory(testInventoryItemId, 10, "COOKING", testMemberId)
      ).rejects.toThrow();
    });
  });
  ...
  describe("deleteInventoryItem", () => {
    ...
    it("should throw error for non-existent item", async () => {
      await expect(inventoryTracker.deleteInventoryItem("invalid-id")).rejects.toThrow();
    });
  });
