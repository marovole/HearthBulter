...
describe("OcrService", () => {
  ...
  describe("recognize", () => {
    ...
    it("应该拒绝不支持的文件类型", async () => {
      ...
      await expect(OcrService.recognize(buffer, "text/plain" as any)).rejects.toThrow(
        "不支持的文件类型"
      );
    });
    ...
    it("应该处理空缓冲区", async () => {
      ...
      // PDF为空时应抛出错误
      await expect(OcrService.recognize(emptyBuffer, "application/pdf")).rejects.toThrow();
    });
  });
