...
import { sensitiveFilterService } from '@/lib/services/sensitive-filter';
...
describe('sensitive-filter', () => {
  let service: sensitiveFilterService;

  beforeEach(() => {
    jest.clearAllMocks();
    service = sensitiveFilterService.getInstance();
  });

  describe('Initialization', () => {
    it('should create service instance', () => {
      expect(service).toBeDefined();
      expect(service).toBeInstanceOf(sensitiveFilterService);
    });

    it('should return same instance (singleton)', () => {
      const instance2 = sensitiveFilterService.getInstance();
