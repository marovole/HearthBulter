...
describe("SensitiveFilterService", () => {
  ...
  describe("filter - Mask Modes", () => {
    ...
    it("should use unique masks for different types", () => {
      ...
      expect(result.filteredText).toContain("110105****002X");
      expect(result.filteredText).toContain("****5678");
    });
  });
  ...
  describe("calculateRiskLevel", () => {
    ...
    it("should return low for low severity items", () => {
      const lowSeverityPattern: SensitiveInfoPattern = {
        type: "custom",
        pattern: /LOW123/g,
        replacement: "***LOW***",
      ...
      const result = sensitiveFilter.filter("LOW123", {
        customPatterns: [lowSeverityPattern],
      });
