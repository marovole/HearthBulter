...
describe('路径遍历防护测试', () => {
  ...
  it.each(pathTraversalPayloads)(
    ...
    (payload) => {
      ...
      expect(containsTraversal || payload.startsWith('/') || payload.includes(':')).toBe(true);
    }
  );
});
...
describe('原型污染防护测试', () => {
  ...
  it.each(prototypePollutionPayloads)(
    '应该防止原型污染',
    (_payload) => {
      // 验证原型未被污染
