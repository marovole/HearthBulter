...
import { IndicatorType, IndicatorStatus } from "@/lib/types/medical";
...
const INDICATOR_DEFINITIONS: IndicatorDefinition[] = [
  // 血脂相关
  {
    type: IndicatorType.TOTAL_CHOLESTEROL,
    patterns: [
    ...
    normalRange: { max: 5.2 },
    getStatus: (value) => {
      if (value > 6.2) return IndicatorStatus.CRITICAL;
      if (value > 5.2) return IndicatorStatus.HIGH;
      return IndicatorStatus.NORMAL;
    },
  },
  {
    type: IndicatorType.LDL_CHOLESTEROL,
    patterns: [
    ...
    normalRange: { max: 3.4 },
    getStatus: (value) => {
      if (value > 4.1) return IndicatorStatus.CRITICAL;
      if (value > 3.4) return IndicatorStatus.HIGH;
      return IndicatorStatus.NORMAL;
    },
  },
  {
    type: IndicatorType.HDL_CHOLESTEROL,
    patterns: [
    ...
    normalRange: { min: 1.0 },
    getStatus: (value) => {
      if (value < 0.9) return IndicatorStatus.LOW;
      return IndicatorStatus.NORMAL;
    },
  },
  {
    type: IndicatorType.TRIGLYCERIDES,
    patterns: [
    ...
    normalRange: { max: 1.7 },
    getStatus: (value) => {
      if (value > 2.3) return IndicatorStatus.CRITICAL;
      if (value > 1.7) return IndicatorStatus.HIGH;
      return IndicatorStatus.NORMAL;
    },
  },
  ...
  // 血糖相关
  {
    type: IndicatorType.FASTING_GLUCOSE,
    patterns: [
    ...
    getStatus: (value) => {
      if (value >= 7.0 || value < 3.0) return IndicatorStatus.CRITICAL;
      if (value >= 6.1 || value < 3.9) return IndicatorStatus.HIGH;
      return IndicatorStatus.NORMAL;
    },
  },
  {
    type: IndicatorType.POSTPRANDIAL_GLUCOSE,
    patterns: [
    ...
    normalRange: { max: 7.8 },
    getStatus: (value) => {
      if (value >= 11.1) return IndicatorStatus.CRITICAL;
      if (value >= 7.8) return IndicatorStatus.HIGH;
      return IndicatorStatus.NORMAL;
    },
  },
  {
    type: IndicatorType.GLYCATED_HEMOGLOBIN,
    patterns: [
      /糖化血红蛋白[：:]\s*(\d+\.?\d*)\s*%/gi,
      /HbA1c[：:]\s*(\d+\.?\d*)\s*%/gi,
      /HbA1C[：:]\s*(\d+\.?\d*)\s*%/gi,
    ...
    normalRange: { max: 6.5 },
    getStatus: (value) => {
      if (value >= 8.0) return IndicatorStatus.CRITICAL;
      if (value >= 6.5) return IndicatorStatus.HIGH;
      return IndicatorStatus.NORMAL;
    },
  },
  ...
  // 肝功能相关
  {
    ...
    normalRange: { max: 40 },
    getStatus: (value) => {
      if (value > 120) return IndicatorStatus.CRITICAL;
      if (value > 40) return IndicatorStatus.HIGH;
      return IndicatorStatus.NORMAL;
    },
  },
  {
    type: IndicatorType.AST,
    patterns: [
  ...
  {
    type: IndicatorType.TOTAL_BILIRUBIN,
    patterns: [
    ...
    normalRange: { max: 21 },
    getStatus: (value) => {
      if (value > 34) return IndicatorStatus.CRITICAL;
      if (value > 21) return IndicatorStatus.HIGH;
      return IndicatorStatus.NORMAL;
    },
  },
  {
    type: IndicatorType.DIRECT_BILIRUBIN,
    patterns: [
    ...
    normalRange: { max: 6.8 },
    getStatus: (value) => {
      if (value > 10) return IndicatorStatus.CRITICAL;
      if (value > 6.8) return IndicatorStatus.HIGH;
      return IndicatorStatus.NORMAL;
    },
  },
  {
    type: IndicatorType.ALP,
    patterns: [/碱性磷酸酶[：:]\s*(\d+)\s*U\/L/gi, /ALP[：:]\s*(\d+)\s*U\/L/gi],
    unit: "U/L",
    normalRange: { min: 40, max: 150 },
    getStatus: (value) => {
      if (value > 300 || value < 30) return IndicatorStatus.CRITICAL;
      if (value > 150 || value < 40) return IndicatorStatus.HIGH;
      return IndicatorStatus.NORMAL;
    },
  },
  ...
  // 肾功能相关
  {
    ...
    normalRange: { max: 133 },
    getStatus: (value) => {
      if (value > 200) return IndicatorStatus.CRITICAL;
      if (value > 133) return IndicatorStatus.HIGH;
      return IndicatorStatus.NORMAL;
    },
  },
  {
    type: IndicatorType.UREA_NITROGEN,
    patterns: [
    ...
    getStatus: (value) => {
      if (value > 10) return IndicatorStatus.CRITICAL;
      if (value > 7.1) return IndicatorStatus.HIGH;
      return IndicatorStatus.NORMAL;
    },
  },
  {
    type: IndicatorType.URIC_ACID,
    patterns: [
    ...
    normalRange: { max: 420 },
    getStatus: (value) => {
      if (value > 600) return IndicatorStatus.CRITICAL;
      if (value > 420) return IndicatorStatus.HIGH;
      return IndicatorStatus.NORMAL;
    },
  },
  ...
  // 血常规相关
  {
    type: IndicatorType.WHITE_BLOOD_CELL,
    patterns: [
    ...
    getStatus: (value) => {
      if (value > 15 || value < 2) return IndicatorStatus.CRITICAL;
      if (value > 10 || value < 4) return IndicatorStatus.HIGH;
      return IndicatorStatus.NORMAL;
    },
  },
  {
    type: IndicatorType.RED_BLOOD_CELL,
    patterns: [
    ...
    getStatus: (value) => {
      if (value > 6 || value < 3) return IndicatorStatus.CRITICAL;
      if (value > 5.5 || value < 4) return IndicatorStatus.HIGH;
      return IndicatorStatus.NORMAL;
    },
  },
  {
    type: IndicatorType.HEMOGLOBIN,
    patterns: [
      /血红蛋白[：:]\s*(\d+\.?\d*)\s*g\/L/gi,
      /Hb[：:]\s*(\d+\.?\d*)\s*g\/L/gi,
      /HGB[：:]\s*(\d+\.?\d*)\s*g\/L/gi,
    ...
    getStatus: (value) => {
      if (value > 180 || value < 80) return IndicatorStatus.CRITICAL;
      if (value > 160 || value < 120) return IndicatorStatus.HIGH;
      return IndicatorStatus.NORMAL;
    },
  },
  {
    type: IndicatorType.PLATELET,
    patterns: [
      /血小板[：:]\s*(\d+)\s*×10\^9\/L/gi,
      /PLT[：:]\s*(\d+)\s*×10\^9\/L/gi,
    ...
    getStatus: (value) => {
      if (value > 500 || value < 50) return IndicatorStatus.CRITICAL;
      if (value > 300 || value < 100) return IndicatorStatus.HIGH;
      return IndicatorStatus.NORMAL;
    },
  },
];
      ...
      for (const pattern of def.patterns) {
        ...
        if (matches.length > 0) {
          ...
          try {
            ...
            const isAbnormal = status !== IndicatorStatus.NORMAL;
