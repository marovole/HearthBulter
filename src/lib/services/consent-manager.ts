...
class ConsentManagerService {
  ...
  private async saveUserConsent(consent: UserConsent): Promise<void> {
    try {
      await convexClient.mutation(api.consents.upsert, {
        userId: consent.userId as Id<"users">,
        consentId: consent.consentId,
        granted: consent.granted,
        context: consent.context,
        ipAddress: consent.ipAddress,
        userAgent: consent.userAgent,
        grantedAt: consent.grantedAt.getTime(),
        expiresAt: consent.expiresAt ? consent.expiresAt.getTime() : undefined,
      });
    } catch (error) {
