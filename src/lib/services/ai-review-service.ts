...
export interface ReviewIssue {
  type: 'medical_claim' | 'extreme_advice' | 'incomplete_info' | 'contradictory' | 'sensitive_topic' | 'uncertainty' | 'commercial_bias';
  ...
  private initializeDefaultRules(): void {
    const defaultRules: ReviewRule[] = [
      ...
      // 极端建议检查
      {
        ...
        pattern: /(?:完全(?:停止|戒除|禁止)|立即(?:停止|戒除)|绝对(?:不能|禁止)|必须完全|严格(?:禁止|戒除)|永久(?:戒除|停止))/gi,
        condition: (content) => this.containsKeywords(content, ['完全停止', '立即停止', '绝对不能', '永久戒除']),
        severity: 'high',
        type: 'extreme_advice',
      ...
      // 敏感话题处理
      {
        ...
        condition: (content) => this.containsKeywords(content, ['癌症', '肿瘤', '艾滋病', '精神疾病']),
        severity: 'high',
