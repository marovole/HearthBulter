...
class SensitiveFilterService {
  private defaultPatterns: SensitiveInfoPattern[] = [
    ...
    {
      type: "age",
      pattern: /(?:年龄|岁数|出生|今年)[\s:：]*\d{1,3}(?:\s*岁)?/g,
      replacement: "***年龄已隐藏***",
      description: "年龄信息",
      severity: "medium",
    },
    ...
    {
      type: "name",
      pattern: /(?:患者|姓名|名字|我叫|我是|好的)[\s:：,，]*[\u4e00-\u9fa5]{2,4}(?:先生|女士)?/g,
      replacement: "***姓名已隐藏***",
      description: "患者姓名",
      severity: "high",
    },
    ...
    for (const match of allMatches) {
      ...
      for (const existing of filteredMatches) {
        ...
        if (matchStart < existingEnd && matchEnd > existingStart) {
          // 类型优先级：身份证号 > 银行卡号 > 其他
          const typePriority: Record<SensitiveInfoPattern["type"], number> = {
            id_card: 3,
            bank_account: 2,
            phone: 1,
            email: 1,
          ...
          const matchPriority = typePriority[match.pattern.type];
          const existingPriority = typePriority[existing.pattern.type];
  ...
  private generatePartialMask(text: string, type: SensitiveInfoPattern["type"]): string {
    switch (type) {
      ...
      case "phone":
        return text.length >= 11 ? `****${text.slice(-4)}` : "***电话***";
