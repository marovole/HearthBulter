...
export class RecommendationRanker {
  ...
  async rankRecipes(
    ...
    // 应用多样性调整
    const diversifiedRanking = this.applyDiversityAdjustment(
      rankedRecipes,
      context,
    );
  ...
  private calculatePopularityScore(recipe: any): number {
    ...
    const viewScore =
      Math.min(Math.log(recipe.viewCount + 1) / Math.log(10000), 1) * 30;
  ...
  private generateRankingExplanation(
    ...
    const topWeight = Object.entries(weights).reduce((a, b) =>
      weights[a[0] as keyof RecommendationWeights] >
      weights[b[0] as keyof RecommendationWeights]
        ? a
        : b,
    );
  ...
  private applyDiversityPenalty(
    ...
    for (const rec of recommendations) {
      const isTooSimilar = filtered.some(
        (existing) =>
          this.calculateRecommendationSimilarity(rec, existing) >
          similarityThreshold,
      );
  ...
  private calculateRecommendationSimilarity(
    ...
    // 基于推荐理由的相似度
    const reasonSimilarity = this.calculateReasonSimilarity(
      rec1.reasons,
      rec2.reasons,
    );
   ...
   * 计算理由相似度
   */
  private calculateReasonSimilarity(
    reasons1: string[],
    reasons2: string[],
  ): number {
  ...
  private applyTemporalDiversity(
    ...
    return recommendations.map((rec) => {
      const recentSimilarity = this.calculateRecentSimilarity(
        rec,
