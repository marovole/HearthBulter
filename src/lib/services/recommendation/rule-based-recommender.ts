...
export class RuleBasedRecommender {
   ...
   * 计算库存匹配评分
   */
  private async calculateInventoryScore(recipe: any, memberId: string): Promise<number> {
   ...
   * 计算价格匹配评分
   */
  private calculatePriceScore(recipe: any, budgetLimit?: number, userPreference?: any): number {
   ...
   * 计算营养匹配评分
   */
  private async calculateNutritionScore(recipe: any, memberId: string): Promise<number> {
    // 获取用户健康目标
   ...
   * 计算偏好匹配评分
   */
  private calculatePreferenceScore(recipe: any, userPreference: any, context: RecommendationContext): number {
    if (!userPreference) {
    ...
    if (userPreference.preferredIngredients) {
      const preferredIngredients = JSON.parse(userPreference.preferredIngredients);
      const recipeIngredients = recipe.ingredients?.map((ing: any) => ing.food.name) || [];
  ...
  private matchesDietaryType(recipe: any, userPreference: any): boolean {
    const { dietType, isVegetarian, isVegan, isLowCarb, isLowFat, isHighProtein } = userPreference;

    // 检查素食/纯素
    if (isVegetarian || isVegan || dietType === 'VEGETARIAN' || dietType === 'VEGAN') {
      const hasMeat = recipe.ingredients?.some((ing: any) => 
        ['肉类', '鸡肉', '猪肉', '牛肉', '鱼', '虾'].includes(ing.food.category)
      );
