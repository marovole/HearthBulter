...
export class ShareTrackingService {
  ...
  async getUserShareAnalytics(
    ...
    const topPerformingContent = sharedContents
      .map((content) => ({
        ...
        conversionRate:
          content.clicks > 0
            ? (content.conversionsCount / content.clicks) * 100
            : 0,
      }))
    ...
    // 每日趋势
    const dailyTrends = await this.calculateDailyTrends(
      shareTokens,
      startDate,
      endDate,
    );
   ...
   * 更新分享统计
   */
  private async updateShareStatistics(
    shareToken: string,
    eventType: ShareTrackingEventType,
  ): Promise<void> {
  ...
  private async grantInvitationReward(
    ...
    // 可以在这里集成到积分系统和VIP系统
    console.log(
      `用户 ${inviterId} 邀请奖励发放: ${vipDays}天VIP + ${points}积分`,
    );
  }
