import { addDays, isAfter, startOfDay, differenceInDays, subDays } from "date-fns";
import {
...
export class AchievementSystem {
    ...
    const triggers: AchievementTrigger[] = [
      {
        ...
        description: ACHIEVEMENT_TYPE_CONFIGS[AchievementType.CHECK_IN_STREAK].description,
        ...
        rarity: ACHIEVEMENT_TYPE_CONFIGS[AchievementType.CHECK_IN_STREAK].rarity,
        points: ACHIEVEMENT_TYPE_CONFIGS[AchievementType.CHECK_IN_STREAK].points,
      ...
      },
      {
        ...
        description: ACHIEVEMENT_TYPE_CONFIGS[AchievementType.WEIGHT_LOSS].description,
        ...
        conditions: [{ metric: "weightGoalAchieved", operator: "eq", value: 1 }],
      ...
      },
      {
        ...
        description: ACHIEVEMENT_TYPE_CONFIGS[AchievementType.NUTRITION_GOAL].description,
        ...
        conditions: [{ metric: "calorieAccuracyDays", operator: "gte", value: 30 }],
      ...
      },
      {
        ...
        description: ACHIEVEMENT_TYPE_CONFIGS[AchievementType.EXERCISE_TARGET].description,
        ...
        rarity: ACHIEVEMENT_TYPE_CONFIGS[AchievementType.EXERCISE_TARGET].rarity,
        points: ACHIEVEMENT_TYPE_CONFIGS[AchievementType.EXERCISE_TARGET].points,
      ...
      },
      {
        ...
        description: ACHIEVEMENT_TYPE_CONFIGS[AchievementType.HEALTH_MILESTONE].description,
        ...
        rarity: ACHIEVEMENT_TYPE_CONFIGS[AchievementType.HEALTH_MILESTONE].rarity,
        points: ACHIEVEMENT_TYPE_CONFIGS[AchievementType.HEALTH_MILESTONE].points,
        conditions: [{ metric: "monthlyHealthScore", operator: "gte", value: 90 }],
        checkFunction: this.checkMonthlyChampion.bind(this),
      },
      {
        type: AchievementType.COMMUNITY_CONTRIBUTION,
        name: ACHIEVEMENT_TYPE_CONFIGS[AchievementType.COMMUNITY_CONTRIBUTION].label,
        description: ACHIEVEMENT_TYPE_CONFIGS[AchievementType.COMMUNITY_CONTRIBUTION].description,
        icon: ACHIEVEMENT_TYPE_CONFIGS[AchievementType.COMMUNITY_CONTRIBUTION].icon,
        color: ACHIEVEMENT_TYPE_CONFIGS[AchievementType.COMMUNITY_CONTRIBUTION].color,
        rarity: ACHIEVEMENT_TYPE_CONFIGS[AchievementType.COMMUNITY_CONTRIBUTION].rarity,
        points: ACHIEVEMENT_TYPE_CONFIGS[AchievementType.COMMUNITY_CONTRIBUTION].points,
  ...
  async checkAchievements(
    ...
    const existingAchievements = await convexClient.query<Array<Record<string, unknown>>>(
    ...
    const unlockedTypes = new Set(
      existingAchievements.map((achievement) => achievement.type as AchievementType)
    );
  ...
  async unlockAchievement(
    ...
    const now = Date.now();
    const achievementId = await convexClient.mutation<string>(api.achievements.createAchievement, {
      memberId: memberId as Id<"familyMembers">,
      type: trigger.type,
      title: trigger.name,
    ...
    const createdAchievement = await convexClient.query<Record<string, unknown> | null>(
  ...
  async getMemberAchievements(memberId: string): Promise<AchievementRecord[]> {
    const achievements = await convexClient.query<Array<Record<string, unknown>>>(
      api.achievements.listByMember,
      {
        memberId: memberId as Id<"familyMembers">,
      }
  ...
  async getAchievementStats(memberId: string): Promise<Record<string, unknown>> {
  ...
  private async checkFirstLogin(_memberId: string, data?: any): Promise<boolean> {
    if (!data?.loginCount) return false;
    return data.loginCount === 1;
  }

  private async checkSevenDayStreak(_memberId: string, data?: any): Promise<boolean> {
    if (!data?.checkinStreak) return false;
    return data.checkinStreak >= 7;
  }

  private async checkMonthlyChampion(_memberId: string, data?: any): Promise<boolean> {
    if (!data?.monthlyHealthScore) return false;
    return data.monthlyHealthScore >= 90;
  }

  private async checkWeightGoalAchieved(_memberId: string, data?: any): Promise<boolean> {
    if (!data?.weightGoalAchieved) return false;
    return data.weightGoalAchieved === true;
  }

  private async checkRecipeMaster(_memberId: string, data?: any): Promise<boolean> {
    if (!data?.recipeCount) return false;
    return data.recipeCount >= 10;
  }

  private async checkSocialButterfly(_memberId: string, data?: any): Promise<boolean> {
    if (!data?.shareCount) return false;
    return data.shareCount >= 20;
  }

  private async checkPerfectWeek(_memberId: string, data?: any): Promise<boolean> {
    if (!data?.weekPerfectScore) return false;
    return data.weekPerfectScore === true;
  }

  private async checkEarlyBird(_memberId: string, data?: any): Promise<boolean> {
    if (!data?.earlyBreakfastStreak) return false;
    return data.earlyBreakfastStreak >= 30;
  }

  private async checkCalorieChampion(_memberId: string, data?: any): Promise<boolean> {
    if (!data?.calorieAccuracyDays) return false;
    return data.calorieAccuracyDays >= 30;
  }

  private async checkInviteMaster(_memberId: string, data?: any): Promise<boolean> {
  ...
  async calculateCheckinStreak(memberId: string): Promise<number> {
    ...
    const dates = new Set(
      healthData.map((record) => startOfDay(new Date(record.measuredAt as number)).toISOString())
    );
  ...
  private normalizeAchievement(record: Record<string, unknown>): AchievementRecord {
    return {
...
export async function getMemberAchievementList(memberId: string): Promise<AchievementRecord[]> {
  const system = AchievementSystem.getInstance();
