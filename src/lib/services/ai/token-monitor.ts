...
export class TokenMonitor {
  ...
  async recordUsage(usage: Omit<TokenUsage, 'timestamp'>): Promise<void> {
    ...
    console.log(`Token使用记录: ${usage.model} - ${usage.tokens} tokens - $${fullUsage.cost.toFixed(4)}`);
  }
  ...
  async getUserStats(userId: string): Promise<TokenStats> {
    ...
    // 找出最常用的模型
    const modelUsage = userRecords.reduce((acc, record) => {
  ...
  private async checkCostLimits(userId: string): Promise<void> {
    ...
    if (stats.dailyCost >= this.costLimits.dailyLimit * this.costLimits.warningThreshold) {
      if (stats.dailyCost >= this.costLimits.dailyLimit) {
        ...
        console.warn(`用户 ${userId} 接近每日成本限制: $${stats.dailyCost.toFixed(2)} / $${this.costLimits.dailyLimit}`);
      }
    }
    ...
    if (stats.monthlyCost >= this.costLimits.monthlyLimit * this.costLimits.warningThreshold) {
      if (stats.monthlyCost >= this.costLimits.monthlyLimit) {
        ...
        console.warn(`用户 ${userId} 接近每月成本限制: $${stats.monthlyCost.toFixed(2)} / $${this.costLimits.monthlyLimit}`);
      }
    }
  }
  ...
  async getGlobalStats(): Promise<{
    ...
    // 统计模型使用情况
    const modelStats = this.usageRecords.reduce((acc, record) => {
      if (!acc[record.model]) {
    ...
    const topModels = Object.entries(modelStats)
      .map(([model, stats]) => ({ model, usage: stats.tokens, cost: stats.cost }))
