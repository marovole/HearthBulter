...
export class DingdongAdapter extends BasePlatformAdapter {
  ...
  async refreshToken(refreshToken: string): Promise<TokenInfo> {
    try {
      ...
      throw new PlatformError({
        ...
        message: `Failed to refresh Dingdong token: ${this.getErrorMessage(error)}`,
  ...
  async searchProducts(
    ...
    try {
      ...
      throw new PlatformError({
        ...
        message: `Failed to search products on Dingdong: ${this.getErrorMessage(error)}`,
        details: { originalError: this.getErrorMessage(error) },
      });
    }
  }
  ...
  async queryStock(request: StockQueryRequest, token: string): Promise<StockQueryResponse> {
    try {
      ...
      throw new PlatformError({
        type: PlatformErrorType.PLATFORM_ERROR,
        message: `Failed to query stock from Dingdong: ${this.getErrorMessage(error)}`,
        details: { originalError: this.getErrorMessage(error) },
      });
    }
  }
  ...
  async createOrder(request: CreateOrderRequest, token: string): Promise<CreateOrderResponse> {
    try {
      ...
      throw new PlatformError({
        type: PlatformErrorType.PLATFORM_ERROR,
        message: `Failed to create order on Dingdong: ${this.getErrorMessage(error)}`,
        details: { originalError: this.getErrorMessage(error) },
      });
    }
  }
  ...
  async getOrderStatus(orderId: string, token: string): Promise<OrderStatusResponse> {
    try {
      ...
      throw new PlatformError({
        type: PlatformErrorType.PLATFORM_ERROR,
        message: `Failed to get order status from Dingdong: ${this.getErrorMessage(error)}`,
        details: { originalError: this.getErrorMessage(error) },
      });
    }
  }
  ...
  async cancelOrder(orderId: string, token: string): Promise<boolean> {
    try {
      ...
      throw new PlatformError({
        type: PlatformErrorType.PLATFORM_ERROR,
        message: `Failed to cancel order on Dingdong: ${this.getErrorMessage(error)}`,
        details: { originalError: this.getErrorMessage(error) },
      });
    }
  }
  ...
  async getProductPrices(productIds: string[], token: string): Promise<Record<string, number>> {
    try {
      ...
      throw new PlatformError({
        type: PlatformErrorType.PLATFORM_ERROR,
        message: `Failed to get product prices from Dingdong: ${this.getErrorMessage(error)}`,
        details: { originalError: this.getErrorMessage(error) },
      });
    }
  }
  ...
  async getDeliveryOptions(address: DeliveryAddress, token: string): Promise<Record<string, any>> {
    try {
      ...
      throw new PlatformError({
        type: PlatformErrorType.PLATFORM_ERROR,
        message: `Failed to get delivery options from Dingdong: ${this.getErrorMessage(error)}`,
        details: { originalError: this.getErrorMessage(error) },
      });
    }
  }
  ...
  async estimateDeliveryTime(
    ...
    try {
      ...
      throw new PlatformError({
        type: PlatformErrorType.PLATFORM_ERROR,
        message: `Failed to estimate delivery time from Dingdong: ${this.getErrorMessage(error)}`,
        details: { originalError: this.getErrorMessage(error) },
      });
    }
  }
