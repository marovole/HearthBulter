...
export class StructuredLogger {
  ...
  private shouldLogToConsole(level: LogLevel): boolean {
    ...
    const configLevelPriority = levelPriorities[this.getLogLevel()];
    const messageLevelPriority = levelPriorities[level];

    return messageLevelPriority >= configLevelPriority;
  }
...
// 导出常用便捷方法
export const log = {
  trace: (message: string, context?: LogContext, metadata?: Record<string, any>) =>
