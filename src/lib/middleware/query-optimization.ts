...
class QueryOptimizer {
  ...
  private getModelClient<T>(model: string): {
    ...
    return {
      findMany: (options) => modelClient.findMany(options) as Promise<T[]>,
      count: modelClient.count,
    };
  }
  ...
  async optimizedFindMany<T>(
    ...
    try {
      ...
      const modelClient = this.getModelClient<T>(model);
      // 执行查询（带超时）
      ...
      this.recordQueryMetrics(model, duration, 0, { error: errorMessage });

      if (errorName === "QueryTimeoutError") {
