...
const PRODUCTION_ENV_CONFIG: EnvConfig = {
  ...
  USDA_API_KEY: {
    ...
    type: "string",
    minLength: 10,
    description: "USDA API 密钥（可选）",
  },
  UPSTASH_REDIS_REST_URL: {
    required: false,
    type: "url",
    description: "Upstash Redis 服务URL（可选）",
  },
  UPSTASH_REDIS_REST_TOKEN: {
    required: false,
    type: "string",
    minLength: 10,
    description: "Upstash Redis 认证令牌（可选）",
  },
};
...
export class EnvSecurityManager {
  ...
  private validateCriticalEnvVars(): void {
    ...
    for (const [key, rule] of Object.entries(this.config)) {
      ...
      try {
        ...
        if (rule.required) {
          errors.push(`必需的环境变量 ${key}: ${message}`);
        } else {
          warnings.push(`可选的环境变量 ${key}: ${message}`);
        }
      }
    }
    ...
    // 记录验证结果
    if (errors.length > 0) {
      logger.error("环境变量验证失败", new Error(errors.join("; ")), undefined, {
        type: "environment",
        errors,
        warnings,
      });
      ...
      logger.warn("环境变量验证警告", undefined, {
        type: "environment",
        warnings,
      });
    } else {
      logger.info("环境变量验证通过", undefined, {
        type: "environment",
  ...
  private checkSecurityIssues(): void {
    ...
    // 检查 Clerk 密钥
    if (process.env.CLERK_SECRET_KEY && process.env.CLERK_SECRET_KEY.length < 20) {
    ...
    if (issues.length > 0) {
      logger.warn("发现环境变量安全问题", undefined, {
        type: "security",
        issues,
  ...
  get(key: string): string | undefined {
    ...
    if (SENSITIVE_KEYS.includes(key) && !this.hasAccessToSensitive(key)) {
      logger.warn("未授权访问敏感环境变量", undefined, {
        type: "security",
        key,
        context: "env_access",
      });
  ...
  validateIntegrity(): boolean {
    try {
      ...
      logger.error("环境变量完整性验证失败", error as Error, undefined, {
        type: "environment",
      });
      return false;
    }
  ...
  set(key: string, value: string): void {
    if (process.env.NODE_ENV === "production") {
      logger.warn("生产环境不允许动态更新环境变量", undefined, {
        type: "security",
        key,
      });
      return;
