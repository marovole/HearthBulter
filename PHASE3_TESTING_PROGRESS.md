...
**扩展的模型 (src/__tests__/setup.ts:304-350)**
- ✅ `healthGoal` - 健康目标查询支持
...
**添��的 Prisma 方法 (src/__tests__/setup.ts:393-396)**
- ✅ `$queryRaw` - 原始SQL查询
- ✅ `$executeRaw` - 原始SQL执行
...
**添加的 Prisma 枚举 (src/__tests__/setup.ts:149-179)**
```typescript
- AchievementType (6个值)
...
**Rate Limiter (src/__tests__/setup.ts:450)**
- ✅ 添加 `getGlobalStats()` 方法
- ✅ 完善统计功能mock

**Device Sync Service (src/__tests__/setup.ts:473-480)**
...
4. **src/__tests__/recommendation-engine.test.ts:31-42**
   - ✅ 扩展 mockPrisma 添加：
     - healthGoal, healthData, familyMember 模型

5. **src/__tests__/notification-system.test.ts:2**
   - ✅ 修复模块导入路径
